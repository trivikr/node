...
class Glob {
  ...
  constructor(pattern, options = kEmptyObject) {
    ...
    if (exclude != null) {
      ...
      if (ArrayIsArray(exclude)) {
        ...
        this.#excludeMatchers = ArrayPrototypeMap(exclude, (pattern) => {
          const path = resolve(this.#root, pattern);
          const matcher = createMatcher(path);
          return {
            __proto__: null,
            matcher,
            nocaseMatcher: matcher.hasMagic() ? null :
              createMatcher(path, { nocaseMagicOnly: false }),
            path,
          };
        });
        this.#isExcluded = (value) => ArrayPrototypeSome(
  ...
  #isExcludedWithCase(value, usePromises = false) {
    if (this.#isExcluded(value)) {
      return true;
    }
    let path;
    for (let i = 0; i < this.#excludeMatchers.length; i++) {
      const exclude = this.#excludeMatchers[i];
      if (exclude.nocaseMatcher?.match(value)) {
        path = exclude.path;
        break;
      }
    }
    ...
    if (!usePromises) {
      const identity = isWindows ?
        this.#cache.identityRealpathSync(path) :
        this.#cache.identitySync(path);
      return identity !== null && identity === (isWindows ?
        this.#cache.identityRealpathSync(value) :
        this.#cache.identitySync(value));
    }
    const identity = isWindows ?
      this.#cache.identityRealpath(path) :
      this.#cache.identity(path);
    return PromisePrototypeThen(identity, (identity) => identity !== null &&
      PromisePrototypeThen(isWindows ?
        this.#cache.identityRealpath(value) :
        this.#cache.identity(value),
                           (valueIdentity) => identity === valueIdentity));
  }
  ...
  globSync() {
    this.#results.setup(this.#root, (value) => this.#isExcludedWithCase(value));
  ...
  #addSubpatterns(path, pattern) {
    ...
    if (this.#isExcludedWithCase(fullpath)) {
      return;
    }
  ...
  async* glob() {
    ...
    while (this.#queue.length > 0) {
      const item = ArrayPrototypePop(this.#queue);
      for (let i = 0; i < item.patterns.length; i++) {
        for await (const result of this.#iterateSubpatterns(item.path, item.patterns[i])) {
          const path = this.#withFileTypes ?
            join(result.parentPath, result.name) :
            resolve(this.#root, result);
          if (this.#excludeMatchers.length === 0 ||
              !await this.#isExcludedWithCase(path, true)) {
            yield result;
          }
        }
      }
  ...
  async* #iterateSubpatterns(path, pattern) {
    ...
    if (this.#excludeMatchers.length > 0 &&
        await this.#isExcludedWithCase(fullpath, true)) {
      return;
    }
    if (isFirst && isWindows && typeof pattern.at(0) === 'string' && StringPrototypeEndsWith(pattern.at(0), ':')) {
      // Absolute path, go to root
