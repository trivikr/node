...
class VFSFileHandle {
  ...
  close() {
    ...
    const promise = h?.close(this.fd);
    return promise ?? PromiseResolve();
  }

  closeSync() {
    const h = vfsState.handlers;
    const result = h?.closeSync(this.fd);
...
async function readFileHandleWithUserBuffer(filehandle, options, size) {
  ...
  while (totalRead < buffer.byteLength) {
    ...
    let bytesRead;
    if (h !== null) {
      const promise = h.read(filehandle.fd, buffer, totalRead, length, -1);
      if (promise !== undefined) bytesRead = await promise;
    }
    bytesRead ??= (await PromisePrototypeThen(
  ...
  if (size === 0) {
    ...
    let bytesRead;
    if (h !== null) {
      const promise = h.read(filehandle.fd, extraBuffer, 0, 1, -1);
      if (promise !== undefined) bytesRead = await promise;
    }
    bytesRead ??= (await PromisePrototypeThen(
...
async function readFileHandle(filehandle, options) {
  ...
  let stats;
  if (h !== null) {
    const promise = h.fstat(filehandle.fd, { __proto__: null, bigint: false });
    if (promise !== undefined) stats = await promise;
  }
  const statFields = stats ?? await PromisePrototypeThen(
  ...
  let size = 0;
  let length = 0;
  const mode = stats ? stats.mode : statFields[1/* mode */];
  if ((mode & S_IFMT) === S_IFREG) {
    size = stats ? stats.size : statFields[8/* size */];
  ...
  while (true) {
    ...
    let bytesRead;
    if (h !== null) {
      const promise = h.read(filehandle.fd, buffer, offset, length, -1);
      if (promise !== undefined) bytesRead = await promise;
    }
    bytesRead ??= (await PromisePrototypeThen(
...
async function open(path, flags, mode) {
  ...
  if (h !== null) {
    ...
    if (result !== undefined) return new FileHandle(new VFSFileHandle(await result));
  }
...
async function read(handle, bufferOrParams, offset, length, position) {
  ...
  let bytesRead;
  if (h !== null) {
    const promise = h.read(handle.fd, buffer, offset, length, position);
    if (promise !== undefined) bytesRead = await promise;
  }
  bytesRead ??= (await PromisePrototypeThen(
...
async function readv(handle, buffers, position) {
  ...
  let bytesRead;
  if (h !== null) {
    const promise = h.readv(handle.fd, buffers, position);
    if (promise !== undefined) bytesRead = await promise;
  }
  bytesRead ??= (await PromisePrototypeThen(
...
async function write(handle, buffer, offsetOrOptions, length, position) {
  ...
  if (isArrayBufferView(buffer)) {
    ...
    let bytesWritten;
    if (h !== null) {
      const promise = h.write(handle.fd, buffer, offset, length, position);
      if (promise !== undefined) bytesWritten = await promise;
    }
    bytesWritten ??= (await PromisePrototypeThen(
  ...
  let bytesWritten;
  if (h !== null) {
    const data = Buffer.from(buffer, length);
    const promise = h.write(handle.fd, data, 0, data.byteLength, offset);
    if (promise !== undefined) bytesWritten = await promise;
  }
  bytesWritten ??= (await PromisePrototypeThen(
...
async function writev(handle, buffers, position) {
  ...
  let bytesWritten;
  if (h !== null) {
    const promise = h.writev(handle.fd, buffers, position);
    if (promise !== undefined) bytesWritten = await promise;
  }
  bytesWritten ??= (await PromisePrototypeThen(
...
async function ftruncate(handle, len = 0) {
  ...
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.ftruncate(handle.fd, len);
    if (promise !== undefined) { await promise; return; }
  }
  return await PromisePrototypeThen(
...
async function fdatasync(handle) {
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.fdatasync(handle.fd);
    if (promise !== undefined) { await promise; return; }
  }
  return await PromisePrototypeThen(
...
async function fsync(handle) {
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.fsync(handle.fd);
    if (promise !== undefined) { await promise; return; }
  }
  return await PromisePrototypeThen(
...
async function fstat(handle, options = { __proto__: null, bigint: false }) {
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.fstat(handle.fd, options);
    if (promise !== undefined) return await promise;
  }
  const result = await PromisePrototypeThen(
...
async function fchmod(handle, mode) {
  ...
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.fchmod(handle.fd, mode);
    if (promise !== undefined) { await promise; return; }
  }
  return await PromisePrototypeThen(
...
async function fchown(handle, uid, gid) {
  ...
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.fchown(handle.fd, uid, gid);
    if (promise !== undefined) { await promise; return; }
  }
  return await PromisePrototypeThen(
...
async function futimes(handle, atime, mtime) {
  ...
  const h = vfsState.handlers;
  if (h !== null) {
    const promise = h.futimes(handle.fd, atime, mtime);
    if (promise !== undefined) { await promise; return; }
  }
  return await PromisePrototypeThen(
