...
function yieldAbortable(source, signal) {
  ...
  return {
    ...
    async *[SymbolAsyncIterator]() {
      ...
      // A single rejection promise and listener are shared by every read of
      // this iteration instead of being re-created per chunk.
      ...
      try {
        while (true) {
          ...
          const { done, value } =
            await SafePromiseRace([iterator.next(), promise]);
          if (done) {
            completed = true;
            return;
          }
        ...
        // Suppress a late rejection if the signal aborts after iteration ends,
        // mirroring the handling of iterator.return() below.
