    ...
    return {
      ...
      [SymbolAsyncIterator]() {
        ...
        return {
          __proto__: null,
          next() {
            const previousNext = state.pendingNext;
            const next = previousNext === null ?
              getNext() :
              PromisePrototypeThen(previousNext, getNext, getNext);
            state.pendingNext = next;
            const onSettled = () => {
              if (state.pendingNext === next) {
                state.pendingNext = null;
              }
            };
            PromisePrototypeThen(next, onSettled, onSettled);
            return next;
          },
