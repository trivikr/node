...
async function closeDuplexChannel(writer, closeIterator) {
  ...
  const endPromise = result < 0 ? closeDuplexWriter(writer) : undefined;
...
async function closeDuplexWriter(writer) {
  if (isPushWriterClosedByConsumer(writer)) return;

  try {
    await writer.end();
  } catch (error) {
    if (!isPushWriterClosedByConsumer(writer)) throw error;
  }
}
