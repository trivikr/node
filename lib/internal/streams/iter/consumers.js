...
function merge(...args) {
  ...
  return {
    ...
    async *[SymbolAsyncIterator]() {
      ...
      const pullNext = (state) => {
        state.pending = true;
        PromisePrototypeThen(
          state.iterator.next(),
          (result) => onSettled(state, result),
          (reason) => onRejected(state, reason),
        );
      };
      ...
      for (let i = 0; i < normalized.length; i++) {
        ...
        const state = { __proto__: null, iterator, pending: false };
        ArrayPrototypePush(iteratorStates, state);
        pullNext(state);
      }
      ...
      try {
        ...
        // On consumer return, await cleanup unless return() is blocked behind
        // an active pull. Primary errors and aborts retain precedence over
...
async function cleanupIterators(
  iteratorStates, primaryError, skipPendingCleanup) {
  let cleanupError = kNoMergeError;
  await SafePromiseAllReturnVoid(iteratorStates, async (state) => {
    const { iterator } = state;
    if (iterator.return) {
      try {
        const result = iterator.return();
        if (primaryError !== kNoMergeError ||
            (skipPendingCleanup && state.pending)) {
