...
class PushQueue {
  ...
  #error = null;
  /** Whether the writer was errored by a normal consumer return */
  #writerClosedByConsumer = false;
  /** Total bytes written */
  #bytesWritten = 0;
  ...
  get writerClosedByConsumer() {
    return this.#writerClosedByConsumer;
  }
  ...
  consumerReturn() {
    ...
    this.#terminateWriterFromConsumer(error, true);
    this.#resolvePendingReads();
  ...
  #terminateWriterFromConsumer(error, returned = false) {
    this.#slots.clear();
    this.#bufferedBytes = 0;
    if (this.#writerState === 'open' || this.#writerState === 'closing') {
      this.#writerState = 'errored';
      this.#error = error;
      this.#writerClosedByConsumer = returned;
    }
    this.#cleanup();
...
let isPushWriterClosedByConsumer;

class PushWriter {
  #queue;

  static {
    isPushWriterClosedByConsumer =
      (writer) => writer.#queue.writerClosedByConsumer;
  }
