...
function pipe(src, dst, finish, finishOnlyHandleError, { end }) {
  if (src._readableState && dst._writableState) {
    return pipeCore(src, dst, finish, { end });
  }
...
function pipeCore(src, dst, finish, { end }) {
  let sourceFinished = false;
  let destinationFinished = false;
  let sourceEnded = isReadableFinished(src);
  let destinationEnded = isWritableFinished(dst, false);
  let sourceClosed = false;
  const waitForSourceClose = willEmitClose(src) && !src.writable;
  const waitForDestinationClose = end && willEmitClose(dst) && !dst.readable;
  ...
  function cleanupSource() {
    ...
    src.removeListener('close', onSourceClose);
  }

  function cleanupDestination() {
    dst.removeListener('finish', onfinish);
    dst.removeListener('error', onDestinationError);
    dst.removeListener('close', onDestinationClose);
  }

  function finishSource(err) {
    if (!sourceFinished) {
      sourceFinished = true;
      cleanupSource();
      finish(err);
  ...
  function finishDestination(err) {
  ...
  function onend() {
    ...
    if (!waitForSourceClose || sourceClosed) {
      finishSource();
    }
  }
  ...
  function onSourceClose() {
    ...
    const rState = src._readableState;
    if (sourceEnded || (rState?.ended && !rState.errored && !rState.errorEmitted)) {
      if (sourceEnded) {
        finishSource();
      }
      return;
    ...
    finishSource(new ERR_STREAM_PREMATURE_CLOSE());
  }

  function onDestinationClose() {
    if (destinationEnded || isWritableFinished(dst, false)) {
      finishDestination();
    } else {
      finishDestination(new ERR_STREAM_PREMATURE_CLOSE());
    }
  }
  ...
  src.once('error', onSourceError);
  src.once('close', onSourceClose);

  if (end) {
    if (destinationEnded) {
      process.nextTick(onfinish);
    } else {
      dst.once('finish', onfinish);
      dst.once('error', onDestinationError);
      dst.once('close', onDestinationClose);
    }
  } else {
    finishDestination();
  }
