...
class ZipProvider extends VirtualProvider {
  ...
  #directoryEntries(name) {
    ...
    const entries = [];
    for (const key of this.#source.keys()) {
      if (StringPrototypeStartsWith(key, prefix)) ArrayPrototypePush(entries, key);
    }
    return entries;
  }

  #validateParent(name, path, syscall, recursive = false) {
    let slash = StringPrototypeLastIndexOf(name, '/');
    while (slash !== -1) {
      const parent = StringPrototypeSlice(name, 0, slash);
      if (this.#source.has(parent)) throw createENOTDIR(syscall, path);
      if (this.#isDirectory(parent)) return;
      if (!recursive) throw createENOENT(syscall, path);
      name = parent;
      slash = StringPrototypeLastIndexOf(name, '/');
    }
  }
  ...
  async open(path, flags, mode) {
    ...
    if (!exists) this.#validateParent(name, path, 'open');
  ...
  async mkdir(path, options) {
    ...
    this.#validateParent(name, path, 'mkdir', options?.recursive);
  ...
  async rename(oldPath, newPath) {
    ...
    const names = this.#directoryEntries(oldName);
    if (names.length === 0) throw createENOENT('rename', oldPath);
    if (newName === oldName) return;
    const oldPrefix = `${oldName}/`;
    if (StringPrototypeStartsWith(newName, oldPrefix)) {
      throw createEINVAL('rename', oldPath);
    }

    for (const name of names) {
      const child = await this.#getEntry(name);
      await this.#source.add(`${newName}/${StringPrototypeSlice(name, oldPrefix.length)}`,
                             await child.content(), {
                               mode: child.mode || undefined,
                               modified: child.modified,
                               method: methodOption(child.method),
                             });
    }
    for (const name of names) await this.#source.delete(name);
  }
  renameSync(oldPath, newPath) {
    ...
    for (const name of names) {
      const child = this.#getEntrySync(name);
      this.#source.addSync(`${newName}/${StringPrototypeSlice(name, oldPrefix.length)}`,
                           child.contentSync(), {
                             mode: child.mode || undefined,
                             modified: child.modified,
                             method: methodOption(child.method),
    ...
    for (const name of names) this.#deleteEntrySync(name);
  }
