...
function createVfsStatFs(options) {
  if (options?.bigint) {
    return {
      __proto__: null,
      type: 0n, bsize: 4096n, frsize: 4096n, blocks: 0n,
      bfree: 0n, bavail: 0n, files: 0n, ffree: 0n,
    };
  }
  return {
    __proto__: null,
    type: 0, bsize: 4096, frsize: 4096, blocks: 0,
...
function createVfsHandlers() {
  return {
    ...
    statfsSync(path, options) {
      const pathStr = toPathStr(path);
      if (pathStr === null) return undefined;
      return findVFSWith(pathStr, 'statfs', (vfs, n) => {
        vfs.statSync(n);
        return createVfsStatFs(options);
      });
    },
