  ...
  return {
    ...
    readv(fd, buffers, position) {
      ...
      return PromiseResolve().then(async () => {
        for (let i = 0; i < buffers.length; i++) {
          const buf = buffers[i];
          const pos = position != null ? position + totalRead : position;
          const { bytesRead } = await vfd.entry.read(buf, 0, buf.byteLength, pos);
          totalRead += bytesRead;
          if (bytesRead < buf.byteLength) break;
        }
        return totalRead;
    ...
    writev(fd, buffers, position) {
      ...
      return PromiseResolve().then(async () => {
        for (let i = 0; i < buffers.length; i++) {
          ...
          const pos = position != null ? position + totalWritten : position;
          const { bytesWritten } = await vfd.entry.write(buf, 0, buf.byteLength, pos);
          totalWritten += bytesWritten;
          if (bytesWritten < buf.byteLength) break;
        }
        return totalWritten;
    ...
    promisesOpen(path, flags, mode) {
      ...
      if (pathStr !== null) {
        const r = findVFSForPath(pathStr);
        if (r !== null) {
          return PromiseResolve(r.vfs.openSync(r.normalized, flags, mode));
        }
      }
      return undefined;
    },
