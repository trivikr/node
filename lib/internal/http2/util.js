...
function mapToHeaders(map,
  ...
  for (const keyEntry of ObjectKeys(map)) {
    let value = map[keyEntry];
    if (value === undefined || keyEntry === '')
      continue;
    const key = keyEntry.toLowerCase();
    let isSingleValueHeader = kSingleValueHeaders.has(key);
