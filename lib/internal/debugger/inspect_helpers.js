...
function childDisablesInspectBrk(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === '--') { break; }
    if (arg === '--no-inspect-brk') {
      inspectBrk = false;
      continue;
    }
    if (arg === '--inspect-brk' ||
        StringPrototypeStartsWith(arg, '--inspect-brk=')) {
      inspectBrk = true;
      continue;
    }
    if (arg === '-' || !StringPrototypeStartsWith(arg, '-')) { break; }

    const equalsIndex = StringPrototypeIndexOf(arg, '=');
    const optionName = equalsIndex === -1 ? arg :
      StringPrototypeSlice(arg, 0, equalsIndex);
    let option = options.get(optionName);
    if (option === undefined) {
      const expansion = aliases.get(optionName);
      if (expansion !== undefined) {
        for (const expandedOption of expansion) {
          option = options.get(expandedOption);
    ...
    if (equalsIndex === -1 &&
        option !== undefined &&
        option.type !== kBoolean &&
        option.type !== kNoOp &&
        option.type !== kV8Option) {
      // Skip the value consumed by this Node.js option. A following
      // --no-inspect-brk is still an option rather than a script argument.
      i++;
    }
  }
...
async function launchChildProcess(childArgs, inspectHost, inspectPort,
  ...
  if (childDisablesInspectBrk(childArgs)) {
    throw new ERR_DEBUGGER_STARTUP_ERROR(
      '`--no-inspect-brk` is incompatible with `node inspect`');
  }
